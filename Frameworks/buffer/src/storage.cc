...
namespace ng {
    namespace detail {
        ...
        memory_t::memory_t(_InputIter first, _InputIter last)
            : _helper(std::make_shared<helper_t>(first, last)), _offset(0)
        {}
        ...
        void memory_t::insert(size_t pos, _InputIter first, _InputIter last)
        {
            ...
            ASSERT_LE(static_cast<size_t>(std::distance(first, last)), _helper->available());
        ...
        static int comp_abs(size_t pos, size_t const& offset, size_t const& node)
        {
            return pos < offset ? -1 : (pos == offset ? 0 : +1);
        }
        ...
        bool storage_t::operator==(storage_t const& rhs) const
        {
            ...
            while (lhsIter != lhsEnd && rhsIter != rhsEnd)
            {
                ...
                if (!std::equal(lhsIter->value.bytes() + lhsOffset,
                                lhsIter->value.bytes() + lhsOffset + size,
                                rhsIter->value.bytes() + rhsOffset))
                    return false;
        ...
        std::string storage_t::substr(size_t first, size_t last) const
        {
            ...
            for (auto it = from; it != to; ++it)
            {
                ...
                res.append(it->value.bytes() + i, j - i);
            }
